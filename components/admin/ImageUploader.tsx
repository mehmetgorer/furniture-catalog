...
type ImageUploaderProps = Readonly<{
  ...
  disabled?: boolean;
  /** Path prefix inside the `website-images` bucket (default: categories). */
  storageFolder?: "categories" | "hero-slides" | "site";
}>;
...
export function ImageUploader({ value, onChange, disabled, storageFolder = "categories" }: ImageUploaderProps) {
  ...
  async function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const path = `${storageFolder}/${crypto.randomUUID()}.${safeExt}`;
  ...
  return (
    <div className="space-y-2">
      ...
      {safeValue ? (
        <div className="mt-2 overflow-hidden rounded-xl border border-stone-800 bg-stone-900">
          {/* eslint-disable-next-line @next/next/no-img-element -- Supabase Storage public URLs */}
          <img src={safeValue} alt="" className="max-h-40 w-full object-contain sm:max-h-48" />
        </div>
      ) : null}
    </div>
  );
